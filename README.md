...
class App extends Component {
  componentDidMount() {
    ...
    FCM.getInitFCMData().then((initialData)=>{
      /* initialData will not be null if :
      1- Android, app in background when notification received
      2- Android, app killed when notification received
      3- iOS, app killed when notification received
      */
    })
    ...
    this.notificationUnsubscribe = FCM.on('notification', (notif) => {
      ...
      This function will be called if :
        1- Android and app is in the foreground ( notif['opened_from_tray'] will be set to false )
        2- iOS and app is in the background ( notif['opened_from_tray'] will be set to true )
        3- iOS and app is in the foreground ( notif['opened_from_tray'] will be set to false )
      */
    });
...
### Sending Local notifications WIP (Credits to [react-native-push-notification](https://github.com/zo0r/react-native-push-notification))
...
- When app is not running when user clicks notification, notification data will be passed through ``` FCM.getInitialData().then((notification)=>{})``` or ```FCM.initialData``` (the latter might cause issues in android when notification is received in background)

- When app is running in background (the tricky one, I strongly suggest you try it out yourself)
 - IOS will receive notificaton from `FCMNotificationReceived` event ( ``` FCM.on('notification',(notif)=>{}) ``` )
